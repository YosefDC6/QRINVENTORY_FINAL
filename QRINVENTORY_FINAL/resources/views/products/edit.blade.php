                ...
                <form method="POST" action="{{ route('products.update', $product) }}" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <input type="number" min="0" name="min_stock" value="{{ old('min_stock', $product->min_stock) }}"
                                   class="mt-1 block w-full border rounded-md p-2 text-sm
                                   dark:bg-gray-900 dark:border-gray-700 dark:text-gray-100">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3"
                                  class="mt-1 block w-full border rounded-md p-2 text-sm
                                  dark:bg-gray-900 dark:border-gray-700 dark:text-gray-100">{{ old('description', $product->description) }}</textarea>
                    </div>
                    ...
                    @if ($product->qr_code_path)
                        <div>
                            ...
                            <img src="{{ asset('storage/'.$product->qr_code_path) }}"
                                 alt="QR {{ $product->sku }}"
                                 class="h-32 w-32 border rounded">
                        </div>
                    @endif
